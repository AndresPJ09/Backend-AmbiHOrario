                    ...
                    (
		                SELECT
			                ...
			                (
				                ...
				                WHERE v.DeletedAt IS null AND rv.RoleId = r.Id AND v.ModuleId = m.Id AND rv.DeletedAt IS NULL
				                GROUP BY v.Id, v.Name, v.Route, v.Description
				                FOR JSON PATH
			                ) AS views
		                FROM Modules AS m
		                INNER JOIN Views AS v ON v.ModuleId = m.Id
		                INNER JOIN RoleViews AS rv ON rv.ViewId = v.Id
                        WHERE m.Id = v.ModuleId AND rv.RoleId = r.Id AND rv.DeletedAt IS NULL
